...
def compute_rmse(x, y, truth):
    ...
    x = [max(x) - y for y in x] # convert from generations bp to generations since start. 
...
def bias_variance(x, y, truth):
    ...
    x = x[0]
    x = [max(x) - z for z in x] # convert from generations bp to generations since start. 
